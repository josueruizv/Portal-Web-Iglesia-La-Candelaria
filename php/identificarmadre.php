...
{
	if(isset($_GET['cedulamadreesp']) and (trim($_GET['cedulamadreesp']))!="")
	{
		$registros=mysqli_query($conexion,"SELECT cedula_mad,nombre_mad,apellido_mad FROM madre WHERE cedula_mad='$_GET[cedulamadreesp]'");
		$reg=mysqli_fetch_array($registros);
		?>

		<form name="auxiliarmadre" id="auxiliarmadre" method="post" action="<?php if($_GET['opc']==6) echo '../pages/registroesposo.php#padresesp'; if($_GET['opc']==15) echo '../pages/editar-matrimonio.php?cod='.$codigo.'#padresesp'; ?>">
		<?php
		  	if($_GET['opc']==6)
		  	{
		?>    
		...
		?>  		
			...
			<input type="hidden" name="domesp" value="<?php echo $_GET['domi']?>" />
		    ...
		    <input type="hidden" name="nommadesp" value="<?php echo $reg['nombre_mad']; ?>" />
		    <input type="hidden" name="apemadesp" value="<?php echo $reg['apellido_mad']; ?>" />
		  <?php
		  	if($_GET['opc']==15)
		  	{
		  ?>
...
else
{
	...
	{	
		...
		<form name="auxiliarmadre" id="auxiliarmadre" method="post" action="<?php if($_GET['opc']==1) echo '../pages/bautizo.php#padres'; if($_GET['opc']==2) echo '../pages/comunion.php?fec='.$fec.'&min='.$min.'#padres'; if($_GET['opc']==3) echo '../pages/confirmacion.php?fec='.$fec.'&min='.$min.'#padres'; if($_GET['opc']==4) echo '../pages/registroesposo.php?#padres'; if($_GET['opc']==11) echo '../pages/editar-bautizo.php?cod='.$codigo.'#padres'; if($_GET['opc']==12) echo '../pages/editar-comunion.php?cod='.$codigo.'#padres'; if($_GET['opc']==13) echo '../pages/editar-confirmacion.php?cod='.$codigo.'#padres'; if($_GET['opc']==14) echo '../pages/editar-matrimonio.php?cod='.$codigo.'#padres' ?>">
		  ...
		  <input type="hidden" name="apepad" value="<?php echo $_GET['apellidopadre']; ?>" />
		  <input type="hidden" name="cedmad" value="<?php echo $_GET['cedulamadre']; ?>" />
		  <input type="hidden" name="nommad" value="<?php echo $reg['nombre_mad']; ?>" />
		  <input type="hidden" name="apemad" value="<?php echo $reg['apellido_mad']; ?>" />
		   <?php
